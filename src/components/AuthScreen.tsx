...
import { Lock, Mail, Key, UserPlus, LogIn, AlertCircle, Zap, ShieldCheck, Cpu, Eye, EyeOff, Sparkles } from 'lucide-react';
...
export const AuthScreen: React.FC<AuthScreenProps> = ({ onLogin, language }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const cleanEmail = email.trim().toLowerCase() || 'trader@quantura.ai';
      
      // Attempt cloud sync with timeout
      try {
        ...
        console.warn('Cloud sync skipped or timed out, proceeding locally:', cloudErr);
      }

      // Success - let the user into the terminal instantly
      apiStorage.setItem('app_is_authenticated', 'true');
      apiStorage.setItem('app_email', cleanEmail);
      ...
      <div className="relative bg-slate-900/90 backdrop-blur-xl border border-slate-800/80 rounded-3xl p-6 sm:p-8 shadow-[0_0_50px_rgba(0,0,0,0.8)] overflow-hidden">
        ...
        <div className="text-center mb-6">
          ...
          <div className="space-y-1">
            ...
            <p className="text-[11px] text-slate-400 max-w-xs mx-auto leading-relaxed pt-1">
              {isArabic 
                ? 'سجل بريدك للدخول والمزامنة التلقائية لصفقاتك ومحفظتك من أي متصفح.' 
                : 'Sign in to automatically synchronize your bot state and wallet across devices.'}
            </p>
          </div>
        </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="trader@quantura.ai"
                dir="ltr"
              />
            </div>
          </div>
          ...
          >
            {loading ? (
              ...
              <>
                ...
                <span>{isArabic ? 'تسجيل الدخول إلى المحطة' : 'Enter Trading Terminal'}</span>
              </>
            ) : (
              <>
                <UserPlus className="w-4 h-4" />
                <span>{isArabic ? 'إنشاء حساب وحفظ البيانات' : 'Register Account'}</span>
              </>
            )}
          </button>

          {/* Quick Direct Bypass Access */}
          <button
            type="button"
        ...
        <div className="mt-6 pt-4 border-t border-slate-800/80 flex items-center justify-between text-[10px] text-slate-400">
          <span className="flex items-center gap-1.5">
            ...
            <span>{isArabic ? 'تشفير سحابي آمن' : 'Encrypted Cloud Storage'}</span>
          </span>
          <span className="flex items-center gap-1.5">
